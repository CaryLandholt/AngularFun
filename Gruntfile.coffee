...
module.exports = (grunt) ->
	grunt.initConfig
		...
		rjs:
			scripts:
				angular: './src/scripts/libs/angular.min.js'
				modules: [
					'./src/scripts/libs/angular-resource.min.js'
					'./src/scripts/libs/angular-route.min.js'
				]
				app: './src/scripts/app.coffee'
				bootstrap: './src/scripts/bootstrap.coffee'
	...
	grunt.registerMultiTask 'rjs', 'Renames files based on their hashed contents', ->
		path = require 'path'
		...
		mainOutput = JSON.stringify(main, null, '\t')
		loadsOutput = JSON.stringify(loads, null, '\t')
		bootstrapOutput = "function (require) {\n\tangular.element(this.document).ready(function () {\n\t\trequire(['#{bootstrap}']);\n\t});\n}"
		output =  "(function () {\nrequire(#{mainOutput}, #{loadsOutput}, #{bootstrapOutput});\n}).call(this);"

		grunt.file.write './src/scripts/main.js', output